const menuOpen = document.getElementById("menuOpen");
const menuClose = document.getElementById("menuClose");
const mobileNav = document.getElementById("mobileNav");

menuOpen.addEventListener("click", () => {
  mobileNav.classList.add("active");
});

menuClose.addEventListener("click", () => {
  mobileNav.classList.remove("active");
});

const techData = [
  {
    title: "LAUNCH VEHICLE",
    img: "technology/image-launch-vehicle-portrait.jpg",
    mobileImg:"technology/image-launch-vehicle-landscape.jpg",
    desc: "A launch vehicle or carrier rocket is a rocket-propelled vehicle used to carry a payload from Earth’s surface to space, usually to Earth orbit or beyond. Our WEB-X carrier rocket is the most powerful in operation. Standing 150 metres tall, it’s quite an awe-inspiring sight on the launch pad!"
  },
  {
    title: "SPACEPORT",
    img: "technology/image-spaceport-portrait.jpg",
    mobileImg:"technology/image-spaceport-landscape.jpg",
    desc: "A spaceport or cosmodrome is a site for launching (or receiving) spacecraft, by analogy to the seaport for ships or airport for aircraft. Based in the famous Cape Canaveral, our spaceport is ideally situated to take advantage of the Earth’s rotation for launch."
  },
  {
    title: "SPACE CAPSULE",
    img: "technology/image-space-capsule-portrait.jpg",
    mobileImg:"technology/image-space-capsule-landscape.jpg",
    desc: "A space capsule is an often-crewed spacecraft that uses a blunt-body reentry capsule to reenter the Earth’s atmosphere without wings. Our capsule is where you'll spend your time during the flight. It includes a space gym, cinema, and plenty of other activities to keep you entertained."
  }
];

const steps = document.querySelectorAll(".steps span");

steps.forEach(step => {
  step.addEventListener("click", () => {

    steps.forEach(s => s.classList.remove("active"));
    step.classList.add("active");

    const index = step.dataset.tech;

    document.getElementById("tech-title").textContent = techData[index].title;
    document.getElementById("tech-desc").textContent = techData[index].desc;
    document.getElementById("tech-img").src = techData[index].img;
    document.getElementById("tech-img-mobile").src = techData[index].mobileImg;
  });
});